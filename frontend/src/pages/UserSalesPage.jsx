...
import { getUserSales, confirmOrder, cancelOrder } from '../services/api';
...
const UserSalesPage = () => {
    ...
    const fetchOrders = async () => {
        if (!id) return;
        try {
            const data = await getUserSales(id);
            setOrders(data || []);
        } catch (error) {
    ...
    const handleCancel = async (orderId) => {
        if (!window.confirm('Отменить продажу товара?')) return;
        try {
            await cancelOrder(orderId, "Cancelled by seller");
    ...
    return (
        <div className="orders-page container" style={{ marginTop: '2rem' }}>
            <h1 className="orders-title" style={{ marginBottom: '1.5rem' }}>Мои Продажи 💰</h1>
                        ...
                        return (
                            <div key={order.id} className="order-card" style={{ border: '1px solid #eee', padding: '1.5rem', borderRadius: '12px', background: '#fff' }}>
                                <div className="order-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                    ...
                                    <span className={`status-badge status-${order.status}`} style={{ fontWeight: 'bold', color: order.status === 'cancelled' ? 'red' : order.status === 'completed' ? 'green' : '#3498db' }}>
                                ...
                                <div className="order-body" style={{ display: 'flex', gap: '1.5rem' }}>
                                    ...
                                    <div style={{ minWidth: '150px', borderLeft: '1px solid #eee', paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                        <div style={{ fontSize: '0.9rem', color: '#666', marginBottom: '0.5rem' }}>Покупатель</div>
